...
FruitObj.prototype.draw = function () {
    ...
    for (var i = 0; i < this.num; i++) {
        ...
        if (this.alive[i]) {
            ...
            if (this.l[i] <= 14) {
                var No = this.aneNo[i];
                this.x[i] = ane.headx[No];//果实的位置，对应的海葵
                this.y[i] = ane.heady[No];//果实的位置，对应的海葵的哪个高度
...
FruitObj.prototype.bron = function (i) {
    this.aneNo[i] = Math.floor(Math.random() * ane.num);  //随机海葵位置
    //this.x[i] = ane.headx[aneID];  //果实的位置，对应的海葵
    //this.y[i] = ane.heady[aneID];  //果实的位置，对应的海葵的哪个高度
    this.l[i] = 0;  //果实长度
