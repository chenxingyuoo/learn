...
Almanac.prototype.touchmove = function (e) {
    ...
    var $date_body_main = $date_body_box.find('.date-body-main');
...
Almanac.prototype.init = function init() {
    ...
    function tplDateBody() {
        var html = '';
        for (var i = 0, len = 3; i < len; i++) {
            html = html + '<ul class="date-body-item clearfix flex-center flex-wrap"></ul>';
        }
        return html;
    }
...
Almanac.prototype.forEachDrawCldThree = function forEachDrawCldThree() {
    ...
    for (var i = 0, len = 3; i < len; i++) {
        // debugger;
        // debugger;
        $curdateBodyItem = $(".date-body-item").eq(i);
        if (i === 0) {
            ...
            $('.date-body-main').attr({
                'style': '-webkit-transform:translate(' + -$('.date-body-box').width() + 'px,0)'
            })
        }
    ...
    for (var item = 0, len = 3; item < len; item++) {
        ...
        for (var i = 0; i < 6; i++) {
            for (var j = 0; j < 7; j++) {
                ...
                if ($('li[data-id="' + (j - 1 >= 0 ? j - 1 : 6) + '"]', $dateHead).hasClass('weekend')) {
                    className = "weekend";
                }

                // 如果该单元格已经存在，则不需要重画
                var $li = $('li[data-id="' + gNum + '"]', $curDateBodyItem);
                var hasLi = $li.size() > 0;
...
Almanac.prototype.drawCld = function drawCld($elBox, year, month, funct) {
    ...
    for (i = 0; i < 42; i++) {
        // debugger;
        var $curLi = $elBox.find('li').eq(i);
        var $solarCalendar = $curLi.find('.solar-calendar');
        var $lunarCalendar = $curLi.find('.lunar-calendar');
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            if (cld[sD].isToday) {      // 今日
                $curLi.addClass("today");         //今日颜色
                // this.showAlmanacDetail(eDay);   // 显示右侧黄历
            }

            if (cld[sD].lDay == 1) {  //显示农历月
                $lunarCalendar.html('<b>' + (cld[sD].isLeap ? '闰' : '') + cld[sD].lMonth + '月' + (this.monthDays(cld[sD].lYear, cld[sD].lMonth) == 29 ? '小' : '大') + '</b>');
            } else {
            ...
            //国历节日
                ...
                if (s.length > 0) {
                    ...
                    s == '黑色星期五' ? $curLi.addClass('black-style') : $curLi.addClass('solar-style');
                }
                //廿四节气
                else {
                    s = cld[sD].solarTerms;
                    if (s.length > 0)  $curLi.addClass('lunar-style');
