...
Almanac.prototype.changeView = function changeView() {
    debugger;
    this.options.currYear = this.currYear;
    this.options.currMonth = this.currMonth;
    this.clear();
...
Almanac.prototype.DateSelection = {
    currYear: -1,
    currMonth: -1,
    ...
    init: function (year, month) {
        ...
        this.DateSelection.setYear.call(this.DateSelection, year);
        this.DateSelection.setMonth.call(this.DateSelection, month);
    },
...
Almanac.prototype.initRiliIndex = function initRiliIndex() {
    ...
    // debugger;
    this.DateSelection.init.apply(this);
};
...
Almanac.prototype.initControlBar = function initControlBar() {
    ...
    for (i = 1901; i <= 2100; i++) {
        if (i == tY)
            htmlYear = htmlYear + '<option val="' + i + '" selected="selected">' + i + '年</option>';
        else {
            htmlYear = htmlYear + '<option val="' + i + '">' + i + '年</option>';
        }
    }
    ...
    for (i = 1; i <= 12; i++) {
        if (i == tM)
            htmlMonth += '<option val="' + i + '" selected="selected">' + i + '月</option>';
        else {
            htmlMonth += '<option val="' + i + '">' + i + '月</option>';
        }
    }
...
Almanac.prototype.bindEvent = function bindEvent(){
    var $doc = $(document);
    //新年列表改变
    $doc.on('change','.year-list',function (){

    });

    //月份列表改变
    $doc.on('change','.month-list',function (){

    });

    // 返回今天
    $doc.on('click','#today-btn',function (){
        this.DateSelection.goToday();
    }.bind(this));

    //上一月
    $doc.on('click','#prevMonth',function (){
        this.DateSelection.goPrevMonth();
    }.bind(this));

    //下一月
    $doc.on('click','#nextMonth',function (){
        this.DateSelection.goNextMonth();
    }.bind(this));
...
Almanac.prototype.init = function init() {
    this.setConfig();
    this.initRiliIndex();
    this.drawCld(this.today.tY(), this.today.tM(), this.options.afterDrawCld);
...
Almanac.prototype.setConfig = function () {
