<template>
  <div class="comment-main">
    ...
    <transition-group v-for="(i , index) in comment" tag="div" name="list" mode="out-in" >
      <div class="comment"  v-if='!i.fid'  :key="i.nickname + i.objectId" >
      ...
      <div class="fcomment"  v-for="j in comment" v-if = 'j.fid == i.objectId'  :key="i.nickname + i.objectId + index">
        <div class="comment-pep" :key=" 'j' + j.nickname + j.objectId">
          <i class="iconfont icon-myline"></i>
          <!--<img src="../assets/pep.png">-->
          <p>{{ j.nickname }}</p>
        </div>
      ...
      <div class="comment-pep-right">
        <div v-if='isShow' class='comment-text'>
          <transition  name="fade">
              <div class="pattern" v-if='pattern.pattern_nickname'>{{ pattern.pattern_nickname }}</div>
          </transition>
          <input type="text" placeholder="您的昵称" v-model="nickname" debounce="500" >
          <transition name="fade" >
            <div class="pattern" v-if='pattern.pattern_email' >{{ pattern.pattern_email }}</div>
          </transition>
        ...
        <div class="comment-footer" >
          ...
          <a name='content'
             class="comment-click"
             :class=" commentClickDisabled ? 'commentClickDisabled' : ''"
             @click='setcomment()'>{{ user }}
            <transition name="fade">
              <span v-if='isCancel'
                    @click.stop='cancel()'
