<template>
  <header>
    <div class="hearder-fixed" :class=" isShowFixed ? 'hearder-fixed-active' : 'hearder-absoleteve'">
      ...
      <!--</a>-->
      <router-link :to="{ name: 'index'}" class="hearder-icon">
        <img src="../assets/head.jpeg" alt="logo">
      </router-link>
      <ul class="navbar">
        <li>
          <!--<a v-link="{name: 'me'}">我的资料</a>-->
          <router-link :to="{ name: 'me'}">我的资料</router-link>
        </li>
        <li v-if='$route.meta.isindex'>
          <a @click.stop="changeShowTitle()" class="search"><i class="iconfont icon-xiazai15"></i></a>
          <transition name="search">
              <input type="text" v-model="title" @keyup.enter="searchtitle(title)" v-if='isShowTitle' />
          </transition>
        </li>
      </ul>
    </div>
    <div class="hearder-content">
      <p class="header-title">{{ $route.isArticleDetails ? topic.title : $route.isMe ? '我的资料' : isSearch ? '搜索结果如下' :
        listname.name}}</p>
    </div>
  </header>
</template>
